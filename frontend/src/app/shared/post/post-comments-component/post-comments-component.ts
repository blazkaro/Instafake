...
import { TuiButton, TuiDialogContext, TuiHintDirective, TuiInput, TuiLabel, TuiLoader, TuiNotificationService } from '@taiga-ui/core';
...
@Component({
  ...
  imports: [PostCommentComponent, TuiInput, TuiTextareaComponent, TuiButton, TuiLabel, TuiHintDirective, TuiLoader, FormsModule],
  templateUrl: './post-comments-component.html',
  styleUrl: './post-comments-component.scss',
  providers: [{
    provide: COMMENTS_POST_ID, useFactory: () => {
      const context = injectContext<TuiDialogContext<boolean, Post>>();
      return context.data.id;
    }
  }, PostCommentsListService, InfiniteScrollService]
})
export class PostCommentsComponent {
  ...
  constructor() {
    effect(() => {
      this.infiniteScrollService.observeSentinel(this.sentinel(), () => this.commentsListService.loadMore());
    });
  }
